...
if __name__ == '__main__':
    ...
    try:
        X_train, X_val, X_test, y_test = load_dataset()
        ...
        train_loader, val_loader, test_loader = get_data_loaders(
            X_train, X_val, X_test, y_test, batch_size=128
        )
        ...
        for batch_X in train_loader:
            print(f"\nSample train batch shape: {batch_X[0].shape}")
            break

        # Check a batch from the test loader
        for batch_X, batch_y in test_loader:
            print(
                f"Sample test batch shape: {batch_X[0].shape}, labels shape: {batch_y.shape}")
            break
