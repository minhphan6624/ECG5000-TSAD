...
# Model parameters
SEQUENCE_LENGTH = 140 # As per ECG5000 dataset
INPUT_FEATURES = 1    # Each time step has 1 feature
HIDDEN_DIM = 64       # Default hidden dimension for LSTM
LATENT_DIM = 8        # As per paper
...
WEIGHT_DECAY = 1e-4
NUM_EPOCHS = 200
NOISE_STDDEV = 0.05 # For denoising autoencoder
PATIENCE = 3
...
def main():
    ...
    try:
        # Assuming load_dataset and get_data_loaders handle the (batch, seq, features) shape correctly
    ...
    trainer = Trainer(
        ...
        criterion=criterion,
        optimizer=optimizer,
        noise_stddev=NOISE_STDDEV, # Denoising is supported by Trainer
    ...
    # evaluate_model expects model to return (reconstructed, latent)
    reconstruction_errors_test, true_labels_test = evaluate_model(
        model, test_loader, device)

    # Separate errors for threshold calculation
    # Ensure we only use normal samples for threshold calculation if available
    ...
    if len(normal_errors_test) == 0:
        print("Error: No normal samples found in the test set for threshold calculation.")
        return
    if len(anomaly_errors_test) == 0:
        print("Warning: No anomaly samples found in the test set. Metrics might be skewed.")
        # Proceed with threshold calculation if possible, but warn the user.
